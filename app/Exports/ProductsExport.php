...
class ProductsExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    */
...

    {
        return [
            'ID',
            'PRODUCT ID NUMBER','BOTANICAL NAME','COMMON NAME','OTHER PRODUCTS / SERVICES NAME','INCLUDE ON WEBSITE','STATUS','DATE ENTERED','BEST SELLERS','NEW FOR THIS YEAR','TAX FREE','TAGS','PURCHASE OPTIONS','PICKUP INSTRUCTIONS','POT SIZE(A)','INVENTORY_COUNT(A)','LOW_INVENTORY_COUNT(A)','CLICK TO INQUIRE(A)','PRE-ORDER(A)','CONTRACTOR PRICE(A)','RETAIL SALE PRICE(A)','RETAIL LIST PRICE(A)','POT SIZE(B)','INVENTORY_COUNT(B)','LOW_INVENTORY_COUNT(B)','CLICK TO INQUIRE(B)','PRE-ORDER(B)','CONTRACTOR PRICE(B)','RETAIL SALE PRICE(B)','RETAIL LIST PRICE(B)','POT SIZE(C)','INVENTORY_COUNT(C)','LOW_INVENTORY_COUNT(C)','CLICK TO INQUIRE(C)','PRE-ORDER(C)','CONTRACTOR PRICE(C)','RETAIL SALE PRICE(C)','RETAIL LIST PRICE(C)','WHOLESALE SUPPLIER NAME','PLUG / TRAY SIZES','RAW COST RANGE','PURCHASING NOTES','YEAR PURCHASED','SHIPPING NOTES','IMAGE FILE NAMES','PERENNIAL','SHRUB','VINE','GRASS / BAMBOO','HARDY TROPICAL','WATER PLANT','ANNUAL','HOUSE / DECK PLANT','CACTUS / SUCCULENT','SMALL TREE','LARGE TREE','MIN ZONE','MAX ZONE','SUNLIGHT','WATER/RAINFALL','SOIL QUALITY','BLOOM SEASON','FLOWER COLOR','BERRY / FRUIT COLOR','SPRING FOLIAGE COLOR','SUMMER FOLIAGE COLOR','FALL FOLIAGE COLOR','HAS EVERGREEN FOLIAGE','HAS WINTER INTEREST','SCENTED FLOWERS / FOLIAGE','DROUGHT TOLERANCE','WET-FEET TOLERANCE','HUMIDITY TOLERANCE','WIND TOLERANCE','POOR SOIL TOLERANCE','HEIGHT','WIDTH','GROWTH RATE','SERVICE LIFE','MAINTENANCE NEEDS','SPREADING POTENTIAL','YEARLY TRIMMING TIPS','PLANT GROUPING SIZE','BEST SIDE OF HOUSE','EXTREME PLANTING LOCATIONS','ORNAMENTAL FEATURES','SPECIAL LANDSCAPE USES','POSSIBLE PEST PROBLEMS','PLANT LIMITATIONS','SUSTAINABLE GARDEN','NATIVE PLANT GARDEN','BIRD & WILDLIFE GARDEN','BUTTERFLY & BEE GARDEN','LUSH TROPICAL GARDEN','DRY SHADE GARDEN','EDIBLE & MEDICINAL GARDEN','RAIN GARDEN','COLORADO & RUSTIC GARDEN','DESERT, CACTUS,& ROCK GARDEN','VIDEO LINK','DESCRIPTION','HOW TO GROW IN KANSAS','PATENT / TRADEMARK NAMES','PLANT ASSETS AND USES','PLANT PICTURE IN DYNASCAPE Y/N','YEAR ENTERED IN DYNASCAPE'
        ];
    }
    public function map($row): array
    {
        ...
        return [
            ...
            $row->pot_size_a,
            (($row->inventory_count_a==0 || is_null($row->inventory_count_a) || ($row->inventory_count_a===NULL))?"0":$row->inventory_count_a),
            (($row->low_inventory_count_a==0 || is_null($row->low_inventory_count_a) || ($row->low_inventory_count_a===NULL))?"0":$row->low_inventory_count_a),
            ...
            $row->pot_size_b,
            (($row->inventory_count_b==0 || is_null($row->inventory_count_b) || ($row->inventory_count_b===NULL))?"0":$row->inventory_count_b),
            (($row->low_inventory_count_b==0 || is_null($row->low_inventory_count_b) || ($row->low_inventory_count_b===NULL))?"0":$row->low_inventory_count_b),
            ...
            $row->pot_size_c,
            (($row->inventory_count_c==0 || is_null($row->inventory_count_c) || ($row->inventory_count_c===NULL))?"0":$row->inventory_count_c),
            (($row->low_inventory_count_c==0 || is_null($row->low_inventory_count_c) || ($row->low_inventory_count_c===NULL))?"0":$row->low_inventory_count_c),
